...
public class MessageDetailsActivity extends BaseFragment {
    ...
    if (timestamp == 0x7ffffffe) {
      ...
      return timestamp + "\n" +
          LocaleController.formatString(
              ...
              LocaleController.getInstance().getFormatterYear().format(
                  new Date(timestamp * 1000L)),
              LocaleController.getInstance()
                  .getFormatterDayWithSeconds()
  ...
  private void updateRows(boolean fullNotify) {
    ...
    rowDataList.add(
        new RowData(rowCount++, getString(R.string.idRow),
                    Integer.toString(messageObject.messageOwner.id)));
    rowDataList.add(new RowData(rowCount++, getString(R.string.dateRow),
                                formatTime(messageObject.messageOwner.date)));

    if (messageObject.caption != null) {
      rowDataList.add(new RowData(rowCount++, getString(R.string.captionRow),
                                  messageObject.caption.toString()));
    }

    if (messageObject.messageOwner.from_scheduled) {
      rowDataList.add(new RowData(
          rowCount++, getString(R.string.scheduleRow),
          String.valueOf(messageObject.messageOwner.from_scheduled)));
    }

    if (messageObject.messageOwner.silent) {
      rowDataList.add(
          new RowData(rowCount++, getString(R.string.silenceRow),
                      String.valueOf(messageObject.messageOwner.silent)));
    }

    if (TextUtils.isEmpty(messageObject.messageText)) {
      rowDataList.add(new RowData(rowCount++, getString(R.string.textRow),
                                  messageObject.messageText.toString()));
    }

    if (messageObject.messageOwner.edit_date != 0) {
      rowDataList.add(
          new RowData(rowCount++, getString(R.string.editDateRow),
                      formatTime(messageObject.messageOwner.edit_date)));
    }

    if (messageObject.messageOwner.from_id != null) {
      var peer = messageObject.messageOwner.from_id;
      if (peer.channel_id != 0 || peer.chat_id != 0) {
        TLRPC.Chat fromChat = getMessagesController().getChat(
            peer.channel_id != 0 ? peer.channel_id : peer.chat_id);
        rowDataList.add(new RowData(rowCount++, getString(R.string.ownerIdRow),
                                    String.valueOf(fromChat.id)));
        rowDataList.add(new RowData(rowCount++,
                                    getString(R.string.ownerChannelRow),
                                    String.valueOf(fromChat.title)));

      } else if (peer.user_id != 0) {
        TLRPC.User fromUser = getMessagesController().getUser(peer.user_id);
        rowDataList.add(new RowData(rowCount++, getString(R.string.ownerIdRow),
                                    String.valueOf(fromUser.id)));
        String fullName =
            fromUser.first_name +
            (fromUser.last_name != null ? " " + fromUser.last_name : "") +
            (fromUser.username != null ? "\n" + fromUser.username : "");
        rowDataList.add(new RowData(rowCount++, getString(R.string.ownerRow),
                                    fullName, OPEN_PROFILE_ID, fromUser.id));
      }
    }
    ...
    if (messageObject.messageOwner.fwd_from != null &&
      ...
      TLRPC.User user =
          getMessagesController().getUser(forwardFromPeer.user_id);
      rowDataList.add(new RowData(rowCount++, getString(R.string.forwardRow),
                                  user.id + " " + user.username));
    }
    if (messageObject.messageOwner.isDeleted()) {
      rowDataList.add(
          new RowData(rowCount++, getString(R.string.deleteRow), "true"));
    }

    if (MessageObject.getMedia(messageObject.messageOwner) != null &&
        MessageObject.getMedia(messageObject.messageOwner).document != null) {
      for (var attribute : MessageObject.getMedia(messageObject.messageOwner)
                               .document.attributes) {
        if (attribute instanceof TLRPC.TL_documentAttributeFilename) {
          String filename = attribute.file_name;
          rowDataList.add(new RowData(
              rowCount++, getString(R.string.fileNameRow), filename));
        }
      }
    }
    ...
    if (filePath != null) {
      rowDataList.add(
          new RowData(rowCount++, getString(R.string.filePathRow), filePath));
    }
    ...
    if (media != null) {
      ...
      if (id != 0) {
        rowDataList.add(new RowData(rowCount++, getString(R.string.docIdRow),
                                    String.valueOf(id)));
      }
    }
