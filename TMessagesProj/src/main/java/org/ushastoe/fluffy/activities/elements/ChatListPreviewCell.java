...
public class ChatListPreviewCell extends FrameLayout {
  ...
  public ChatListPreviewCell(Context context) {
    ...
    preview = new FrameLayout(context) {
      ...
      protected void onDraw(Canvas canvas) {
        ...
        textPaint.setColor(ColorUtils.blendARGB(
            0x00, Theme.getColor(Theme.key_windowBackgroundWhiteBlackText),
            titleProgress));
        textPaint.setTextSize(AndroidUtilities.dp(20));
        titleText = (String)TextUtils.ellipsize(
            titleText, textPaint,
            w - AndroidUtilities.dp(130 + 35 * statusProgress),
        ...
        Drawable search =
            ContextCompat.getDrawable(context, R.drawable.ic_ab_search)
                .mutate();
        search.setColorFilter(new PorterDuffColorFilter(
            Color.argb(204, r, g, b), PorterDuff.Mode.MULTIPLY));
        search.setBounds(
            (int)w - AndroidUtilities.dp(39), AndroidUtilities.dp(22),
            (int)w - AndroidUtilities.dp(12), AndroidUtilities.dp(49));
        search.draw(canvas);

        if (showStoriesProgress > 0) {
          // Используем общий сдвиг для расчёта положения сторис
          float circleX =
              centeredTitleProgress * titleMoveDelta + AndroidUtilities.dp(60);
          ...
          canvas.drawCircle(circleX, circleY,
                            circleRadius * showStoriesProgress, paint);
          ...
          if (drawable != null) {
            ...
            drawable.setAlpha(
                (int)(255 * showStoriesProgress)); // Добавляем плавное скрытие
        ...
        for (int i = 0; i < 3; i++) {
          ...
          canvas.drawRoundRect(
              AndroidUtilities.dpf2(20), AndroidUtilities.dpf2(start),
              AndroidUtilities.dpf2(20 + 20),
              AndroidUtilities.dpf2(start + 2.8f), AndroidUtilities.dp(10),
              AndroidUtilities.dp(10), Theme.dialogs_onlineCirclePaint);
        }

        // Используем общий сдвиг для расчёта положения заголовка
        float titleStart =
            centeredTitleProgress * titleMoveDelta + AndroidUtilities.dp(78);
        float titleEnd = titleStart + width;

        Theme.dialogs_onlineCirclePaint.setColor(ColorUtils.blendARGB(
            0x00,
            ColorUtils.setAlphaComponent(Theme.getColor(Theme.key_switchTrack),
                                         0x5F),
            titleProgress * statusProgress));
        canvas.drawRoundRect(
            titleEnd + AndroidUtilities.dp(5), AndroidUtilities.dp(22),
            titleEnd + AndroidUtilities.dp(30), AndroidUtilities.dp(47),
            AndroidUtilities.dp(4), AndroidUtilities.dp(4),
            Theme.dialogs_onlineCirclePaint);
        canvas.drawText(titleText, titleStart, AndroidUtilities.dp(42),
                        textPaint);
      }
    };
    ...
    addView(preview, LayoutHelper.createFrame(
                         ...
                         Gravity.TOP | Gravity.CENTER, 21, 15, 21, 21));
  ...
  public void updateStories(boolean animate) {
    ...
    if (animate) {
      animator =
          ValueAnimator.ofFloat(showStoriesProgress, to).setDuration(250);
  ...
  public void updateCenteredTitle(boolean animate) {
    ...
    if (animate) {
      animator =
          ValueAnimator.ofFloat(centeredTitleProgress, to).setDuration(250);
      animator.setInterpolator(Easings.easeInOutQuad);
      animator.addUpdateListener(animation -> {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    super.onMeasure(
        MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(widthMeasureSpec),
                                    MeasureSpec.EXACTLY),
        MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(72 + 15 + 21),
                                    MeasureSpec.EXACTLY));
  }
}
