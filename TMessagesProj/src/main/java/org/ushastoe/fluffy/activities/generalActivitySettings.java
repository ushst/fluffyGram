...
public class generalActivitySettings extends BaseFragment {
  ...
  private enum RowType { HEADER, TEXT_CHECK, TEXT_CELL, SHADOW_SECTION }
  ...
  private void updateRows() {
    ...
    rows.add(new Row(RowIdentifier.GENERAL_HEADER, RowType.HEADER,
                     R.string.General));
    rows.add(new Row(RowIdentifier.DOWNLOAD_SPEED_BOOST, RowType.TEXT_CHECK,
                     R.string.downloadSpeedBoost, R.drawable.msg_download));
    rows.add(new Row(RowIdentifier.SAVE_EDITED, RowType.TEXT_CHECK,
                     R.string.saveEditRow, R.drawable.msg_edit));
    rows.add(new Row(RowIdentifier.SAVE_DELETED, RowType.TEXT_CHECK,
                     R.string.saveDelRow, R.drawable.msg_delete));
    rows.add(new Row(RowIdentifier.UNMUTE_WITH_VOLUME, RowType.TEXT_CHECK,
                     R.string.unmuteVideoWithVolume, R.drawable.media_unmute));
    rows.add(new Row(RowIdentifier.PAUSE_MUSIC_ON_MEDIA, RowType.TEXT_CHECK,
                     R.string.PauseMusicOnMedia,
                     R.drawable.msg_filled_data_music));
    rows.add(new Row(RowIdentifier.BIG_PHOTO_SEND, RowType.TEXT_CHECK,
                     R.string.SendLargePhoto,
                     R.drawable.msg_filled_data_photos_solar));

    rows.add(new Row(RowIdentifier.DIVIDER_1, RowType.SHADOW_SECTION));
    rows.add(new Row(RowIdentifier.VOICE_RECOGNITION_HEADER, RowType.HEADER,
                     R.string.Voip));
    rows.add(new Row(RowIdentifier.VOICE_PROVIDER_SELECTOR, RowType.TEXT_CELL,
                     R.string.UseCloudflare, R.drawable.voicechat_muted));
    if (fluffyConfig.useCloudFlare()) {
      rows.add(new Row(RowIdentifier.VOICE_PROVIDER_CREDENTIALS,
                       RowType.TEXT_CELL, R.string.CloudflareCredentials,
                       R.drawable.msg_voicechat_solar));
    }

    rows.add(new Row(RowIdentifier.EXPERIMENTAL_SETTINGS_HEADER, RowType.HEADER,
                     R.string.Other));
    rows.add(new Row(RowIdentifier.ALLOW_ATTACH_ANY_BOT, RowType.TEXT_CHECK,
                     R.string.AllowAttachAnyBot, R.drawable.msg_bot));
  ...
  private void selectProvider(Context context) {
    ...
    CharSequence[] items = new CharSequence[] {"Telegram", "Cloudflare"};
    ...
    for (int i = 0; i < items.length; ++i) {
      ...
      cell.setTextAndValue(items[index],
                           index == fluffyConfig.voiceUseCloudflare);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
      ...
      case TEXT_CELL:
        ...
        if (row.id == RowIdentifier.VOICE_PROVIDER_SELECTOR) {
          String value =
              fluffyConfig.useCloudFlare() ? "Cloudflare" : "Telegram";
      ...
      case TEXT_CHECK:
        ...
        textCheckCell.setTextAndCheckAndIcon(getString(row.textResId), checked,
                                             row.iconResId, true);
        break;
      }
    }
