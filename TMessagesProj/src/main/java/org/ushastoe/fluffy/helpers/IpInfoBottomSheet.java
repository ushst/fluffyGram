...
public class IpInfoBottomSheet extends BottomSheet {
    ...
    private IpInfoBottomSheet(BaseFragment fragment, String ipAddress) {
        ...
        titleTextView.setText(LocaleController.getString("checkIp", R.string.checkIp));
        ...
        container.addView(titleTextView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, Gravity.TOP));

        // Контейнер для контента (результат или прогресс-бар)
        contentFrame = new FrameLayout(getContext());
        container.addView(contentFrame, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 150, Gravity.TOP, 0, 70, 0, 80));

        // Прогресс-бар
        progressBar = new ProgressBar(getContext());
        contentFrame.addView(progressBar, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER));
        ...
        contentFrame.addView(resultTextView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT));
        ...
        container.addView(buttonView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, Gravity.BOTTOM));

        View buttonShadowView = new View(getContext());
        buttonShadowView.setBackgroundColor(getThemedColor(Theme.key_dialogShadowLine));
        buttonView.addView(buttonShadowView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, AndroidUtilities.getShadowHeight()));
        ...
        closeButton.setTextColor(Theme.getColor(Theme.key_featuredStickers_buttonText));
        ...
        closeButton.setBackground(Theme.AdaptiveRipple.filledRect(Theme.getColor(Theme.key_featuredStickers_addButton), 6));
        closeButton.setOnClickListener(v -> dismiss());
        buttonView.addView(closeButton, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 48, Gravity.BOTTOM | Gravity.FILL_HORIZONTAL, 16, 16, 72, 16));
        ...
        copyButton.setColorFilter(Theme.getColor(Theme.key_featuredStickers_buttonText));
        ...
        copyButton.setOnClickListener(v -> {
            if (!TextUtils.isEmpty(resultTextToCopy)) {
                ...
                // В вашем стиле используется BulletinFactory, можно использовать и его, или простой Toast
                Toast.makeText(getContext(), getString("TextCopied", R.string.TextCopied), Toast.LENGTH_SHORT).show();
            }
        });
        ...
        IpApiHelper.getIpInfo(ipAddress, (ipInfo, exception) -> {
            ...
            AndroidUtilities.runOnUIThread(() -> {
                ...
                if (exception != null) {
                    ...
                    String error = (ipInfo != null && ipInfo.status != null) ? ipInfo.status : "Unknown error";
    ...
    private String buildInfoString(IpApiHelper.IpInfoResponse info) {
        ...
        sb.append("Country: ").append(info.country).append(" (").append(info.countryCode).append(")\n");
