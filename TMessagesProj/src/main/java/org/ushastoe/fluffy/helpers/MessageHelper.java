...
public class MessageHelper {
  ...
  public static String getPathToMessage(MessageObject messageObject) {
    ...
    if (TextUtils.isEmpty(path)) {
      path = FileLoader.getInstance(UserConfig.selectedAccount)
  ...
  public static MessageObject.GroupedMessages
  getValidGroupedMessage(MessageObject message) {
    LongSparseArray<MessageObject.GroupedMessages> groupedMessagesMap =
        new LongSparseArray<>();
    ...
    if (message.getGroupId() != 0) {
      ...
      if (groupedMessages != null &&
          (groupedMessages.messages.size() <= 1 ||
           groupedMessages.getPosition(message) == null)) {
  ...
  public static SpannableResult createNewString(MessageObject messageObject) {
    ...
    final SpannableStringBuilder[] spannedStrings =
        new SpannableStringBuilder[4];
    final Drawable[] icons = {
        messageObject.messageOwner.isDeleted() ? Theme.chat_deleteDrawable
                                               : null,
        messageObject.isEdited() ? Theme.chat_editDrawable : null,
        messageObject.messageOwner.silent ? Theme.chat_silentDrawable : null,
        messageObject.messageOwner.from_scheduled ? Theme.chat_sheduleDrawable
                                                  : null};
    ...
    for (int i = 0; i < icons.length; i++) {
      if (icons[i] != null) {
        ...
        iconSpan.setSize(30);
        spannedStrings[i].setSpan(iconSpan, 0, 1,
